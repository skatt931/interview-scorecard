...
export const QuietInput = ({ className = '', ...props }: ComponentProps<'input'>) => (
  <input {...props} className={`${quiet} ${className}`} />
)

export const TextArea = ({ className = '', ...props }: ComponentProps<'textarea'>) => (
  <textarea {...props} className={`${base} resize-y leading-relaxed ${className}`} />
)
...
export const Button = ({
  ...
  return (
    <button
      {...props}
      className={`rounded-xl px-5 py-2.5 text-sm font-semibold shadow-sm transition duration-200 active:scale-[0.98] disabled:pointer-events-none disabled:opacity-40 ${tone} ${className}`}
    />
  )
}
